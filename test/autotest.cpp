...
Examples:\n\
  " MAIN_TITLE                                                                 \
    " -a                           test with all options, showing all passes and failures\n\
  " MAIN_TITLE                                                                 \
    " -a 1>/dev/null               test with all options, showing only errors, with stdout > null\n\
  " MAIN_TITLE                                                                 \
    " -ap14 1>/dev/null            test with all options and precision limit of 14 for %%f %%e %%g, showing only errors, with stdout > null\n\
  " MAIN_TITLE                                                                 \
    " -ixou 1>/dev/null            test only %%i %%x %%o %%u, showing only errors, with stdout > null\n\
\n\
"
...
static void test_f(void)
{
    ...
    {
        strcat(fmt_buf, ".");
        sprintf(&fmt_buf[strlen(fmt_buf)], "%i",
                1 + rand() % min(width, opts.prec_max));
    };
